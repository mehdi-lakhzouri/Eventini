...
describe('hasOrganizationScope', () => {
  ...
  /**
   * The `via` path, used by exactly one model —
   * `membership_role_assignments`, which DATABASE_SCHEMA.md §5.6 declares
   * tenant-owned "via le membership" and gives no `organization_id`.
   *
   * `via` is supplied by the ownership registry, never read off the clause.
   * That is what stops a model which *does* have the column from being scoped
   * by a join instead, which §2.4 denormalises the column to avoid.
   */
  describe('scoping through a relation', () => {
    it('accepts a nested organization filter', () => {
      expect(
        hasOrganizationScope(
          'findMany',
    ...
    it('accepts the `is` form Prisma also emits', () => {
      expect(
        hasOrganizationScope(
          'findMany',
          { where: { membership: { is: { organizationId: ORG } } } },
          'membership',
        ).scoped,
      ).toBe(true);
    });

    it('accepts it inside an AND', () => {
      expect(
        hasOrganizationScope(
          'findMany',
          {
    ...
    it('accepts a create that names the membership', () => {
      expect(
        hasOrganizationScope(
          'create',
          { data: { membershipId: 'mbr_1', roleId: 'rol_1' } },
          'membership',
        ).scoped,
      ).toBe(true);
      expect(
        hasOrganizationScope(
          'create',
          { data: { membership: { connect: { id: 'mbr_1' } } } },
          'membership',
        ).scoped,
      ).toBe(true);
    });

    it('still refuses a query with no constraint at all', () => {
      const verdict = hasOrganizationScope('findMany', {}, 'membership');

      expect(verdict.scoped).toBe(false);
      expect(verdict.reason).toContain('membership');
    });

    it('refuses a filter on the membership that is not the organization', () => {
      expect(
        hasOrganizationScope(
          'findMany',
          { where: { membership: { userId: 'usr_1' } } },
          'membership',
        ).scoped,
      ).toBe(false);
    });
    ...
    /**
     * Without `via`, the same clause is not a scope. This is the property
     * that keeps the relation form confined to the one model that needs it.
     */
    it('is not accepted for a model that has the column', () => {
      expect(
        hasOrganizationScope('findMany', {
          where: { membership: { organizationId: ORG } },
        }).scoped,
      ).toBe(false);
    });
