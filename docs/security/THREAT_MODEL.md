...
| RA-10 | `brace-expansion` (high, DoS) confiné à `eslint`/`jest` | La seule copie non patchée restante sur chaque projet est la dépendance interne d'ESLint à `minimatch@3.1.5` (`node_modules/minimatch/node_modules/brace-expansion@1.1.x`), plus, côté backend, trois copies internes à Jest. **Aucune version corrigée n'existe pour ces lignes majeures** — le correctif (`GHSA-mh99-v99m-4gvg`) n'a jamais été rétroporté avant la réécriture `5.0.8`, et bumper `eslint`/`jest` vers une version qui l'embarque est une rupture hors périmètre de ce ticket. **Dev-only, jamais exécuté sur une entrée non maîtrisée** : ESLint et Jest ne traitent que les motifs de fichiers écrits dans nos propres scripts npm | Réévaluer à la prochaine montée majeure d'ESLint ou de Jest |
